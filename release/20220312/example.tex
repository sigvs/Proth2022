...
denote  respectively  the  time  to  repair  and  the repair rate for machine $i$, conditioned that this machine is under repair. Under our 
 assumptions,  the  system  has  the  steady  state  mode  (see  e.g. \cite{Seva62}),  and  the performance  of  the  system  in  this  mode  is  important  for 
applications.
...
In many combinatorial optimization problems, local optima of the objective function (or fitness)  tend to be groupped in a ``big valley`` (in  the case of minimization problems) or
...
Excluding the equivalent solutions in view of the problem symmetries may reduce the number of clusters in the GA population.
This has the same effect as the usage of non-degenerate solution encodings~\citep{ReevesDai} and may 
